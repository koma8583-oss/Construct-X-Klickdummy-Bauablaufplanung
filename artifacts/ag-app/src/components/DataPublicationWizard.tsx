...
type PolicyTerm = { label: string; value: string };

function policyTerms(agreement: ContractorOption["parentAgreement"]): PolicyTerm[] {
  ...
  return terms.flatMap(([key, label]) => {
...
export function DataPublicationWizard({ open, onOpenChange, projectId, contractors, takte = [] }: Props) {
  ...
  const parentPolicy = chosen?.parentAgreement;
  const parentPolicyIsConcrete = Boolean(
    parentPolicy &&
    parentPolicy.id &&
    Number.isInteger(parentPolicy.version) &&
    (parentPolicy.version ?? 0) > 0 &&
    parentPolicy.lifecycleStatus === "ACCEPTED" &&
  ...
  const previewPolicy = async () => {
    ...
    try {
      const response = await fetch("/api/leistungsanfragen/policy-preview", {
        ...
        body: JSON.stringify({ taktIds: [...leistungen], nuOrgId: chosen.orgId, purpose, selectedFields: [...fields], parentPolicyId: parentPolicy.id, parentPolicyVersion: parentPolicy.version }),
      });
  ...
  const submit = async () => {
    if (!purpose || !chosen || !parentPolicyIsConcrete || !parentPolicy || submitting) return;
    const selectedParentPolicy = parentPolicy;
